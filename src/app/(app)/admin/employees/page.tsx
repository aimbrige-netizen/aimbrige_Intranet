...
export default async function EmployeesPage({
  ...
  const sortKey: EmployeeSortKey =
    searchParams.sort === "hire_date" ? "hire_date" : "name";
  ...
  let query = supabase
    .from("employees")
    .select(
      `id, name, email, position, phone, hire_date, employment_status, profile_image_url,
       auth_user_id,
    ...
    .order(sortKey === "hire_date" ? "hire_date" : "name", { ascending })
  ...
  const tableRows: EmployeeTableRow[] = employees.map((employee) => ({
    id: employee.id,
    name: employee.name,
  ...
  const sortHref = (key: EmployeeSortKey) => {
    ...
    search.set("sort", key);
    search.set("dir", sortKey === key && ascending ? "desc" : "asc");
      ...
      {error ? (
        ...
        <Card className="overflow-hidden">
          <EmployeeTable
            ...
            sort={{ key: sortKey, dir: ascending ? "asc" : "desc" }}
            sortHref={sortHref}
            sortableKeys={["name", "hire_date"]}
