...
| 12 | `20260730000012_fix_attendance_guard.sql` | 근태 가드 트리거 수정 |

> 08·11·12는 앞선 마이그레이션의 보안 결함을 고칩니다. **반드시 번호 순서대로**
> 실행하세요. 새로 세팅하는 환경에서는 01~12를 순서대로 한 번씩 실행하면 됩니다.
>
> 12가 필요한 이유: 08의 가드 트리거를 `SECURITY DEFINER`로 선언하면 함수 안에서
> `current_user`가 호출자가 아니라 소유자(postgres)로 평가돼, "관리자면 통과"
> 분기가 항상 참이 되어 가드가 한 번도 동작하지 않습니다. 트리거는 반드시
> `SECURITY INVOKER`여야 합니다.
