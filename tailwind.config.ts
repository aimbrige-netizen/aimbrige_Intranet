  ...
  theme: {
    extend: {
      colors: {
        ...
        "line-dark": "#4a4b4c",
        /*
         * 지표 강조 — 민트. 판정이 두 번 뒤집혔다:
         *   04 최초 실측: "지표 강조는 민트 rgb(68,209,165)" → 채택.
         *   06 전수 덤프: gw 변수 632개에 민트가 없음 → 오독으로 판정, 시안으로 통일.
         *   14 ehr 재실측(★판정): **gw 632개 변수엔 없지만 ehr 앱이 자체
         *   팔레트를 얹는다** — ehr 근태 화면 지표 값이 민트 rgb(68,209,165)로
         *   재확인됐다. 04가 옳았다. 민트로 복원한다.
         *
         * 용처 원칙(14-ehr): 브랜드·액션·활성 = primary 시안(#08a7bf),
         * **지표 값·게이지 강조 = accent 민트**. 민트는 흰 배경 대비 1.9:1이라
         * 종전 규칙 그대로 28px급 큰 값·게이지 막대 전용이다 — 작은 글자·칩
         * 범례는 ink(#158466)로 간다. StatCard positive·Progress positive·범례는
         * 이 토큰을 참조하므로 값 교체만으로 민트 축으로 복귀한다.
         */
        accent: {
          DEFAULT: "#44d1a5",
          ink: "#158466",
